...
# Definire SMARTS per i Gruppi Zuccherini
SugarSmarts = [
    '[C@H]1(O[C@@H](O)[C@@H](O)[C@H](O)[C@H]1O)',  # esempio di glucosio
    # Aggiungere altri zuccheri comuni se necessario
    
]
Sugars = [Chem.MolFromSmarts(smarts) for smarts in SugarSmarts]

# Aggiungere parametri di esempio per i gruppi zuccherini
pads1.append([2.0, 150.0, 100.0, 1.0, 20.0, 30.0, 50.0])
pads2.append([2.0, 150.0, 100.0, 1.0, 20.0, 30.0, 50.0])
...
def properties(mol):
    ...
    if (mol is None):
        ...
        sys.exit(1)
    x = [0] * 9  # Aumenta la dimensione dell'array per includere i gruppi zuccherini
    ...
    # Aggiungi il conteggio dei gruppi zuccherini
    for sugar in Sugars:
        if (mol.HasSubstructMatch(sugar)):
            matches = mol.GetSubstructMatches(sugar)
            x[8] += len(matches)
    
    return x

def qed(w, p, gerebtzoff):
    d = [0.00] * 9  # Aumenta la dimensione dell'array per includere gli zuccheri
    if (gerebtzoff):
        for i in range(0, 9):
...
def weights_max(mol, gerebtzoff=True):
    ...
    return qed([0.50, 0.25, 0.00, 0.50, 0.00, 0.50, 0.25, 1.00, 0.50], props, gerebtzoff)
...
def weights_mean(mol, gerebtzoff=True):
    ...
    """
    props = properties(mol)
    return qed([0.66, 0.46, 0.05, 0.61, 0.06, 0.65, 0.48, 0.95, 0.50], props, gerebtzoff)
...
def weights_none(mol, gerebtzoff=True):
    ...
    """
    props = properties(mol)
    return qed([1.00, 1.00, 1.00, 1.00, 1.00, 1.00, 1.00, 1.00, 1.00], props, gerebtzoff)
